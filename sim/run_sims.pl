...
foreach $trace (@traces){
	...
	for ($i = 0; $i < $numPrefetchers; $i++){
		...
		# run the simulation
		`./Simulator/Scripts/run_simulator.py --definition 1-core/stream \
		 --config lru/1m --folder scratch --trace-selector traces/ --workload $trace \
		 --warm-up 0 --run-time 250000000 --prefetcher $i`
